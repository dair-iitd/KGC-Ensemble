...
def train_and_validate(cfg, model, train_data, valid_data, filtered_data=None):
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            # edge_weight = [0 for _ in range(train_data.num_edges)]
            # # print(data.edge_type)
            # for idx, _ in enumerate(train_data.edge_type):
            #     h = train_data.edge_index[0][idx].item()
            #     t = train_data.edge_index[1][idx].item()
            #     r = _.item()
            ...
            for batch in train_loader:
                ...
                # pred = parallel_model(train_data, batch, edge_weight)
...
def test(cfg, model, test_data, filtered_data=None):
    ...
    # edge_weight = [0 for _ in range(test_data.num_edges)]
    # # print(data.edge_type)
    # for idx, _ in enumerate(test_data.edge_type):
    #     h = test_data.edge_index[0][idx].item()
    #     t = test_data.edge_index[1][idx].item()
    #     r = _.item()
    #     if nbf2rnnent is not None:
    #         h = nbf2rnnent[h]
    #         t = nbf2rnnent[t]
    ...
    for batch in test_loader:
        ...
        # t_pred = model(test_data, t_batch, edge_weight)
        # h_pred = model(test_data, h_batch, edge_weight)
...
if __name__ == "__main__":
    ...
    train_and_validate(cfg, model, train_data, test_data, filtered_data=filtered_data)
    if util.get_rank() == 0:
       logger.warning(separator)
       logger.warning("Evaluate on valid")
