...
class NBFNet(nn.Module):
    ...
    def bellmanford(self, data, h_index, r_index, edge_weight=None, separate_grad=False, WN=True):
        ...
        # new_boundary = simkgc.get_h(h_index, r_index)
        # new_boundary = new_boundary.unsqueeze(-1).repeat(1, 1, self.dims[0]).to(h_index.device)

        # boundary += new_boundary

        edge_weight = torch.ones(data.num_edges, device=h_index.device)
        # edge_weight = [0 for _ in range(data.num_edges)]
        # # print(data.edge_type)
        # for idx, _ in enumerate(data.edge_type):
        #     h = data.edge_index[0][idx].item()
        #     t = data.edge_index[1][idx].item()
        #     r = _.item()
        #     if nbf2rnnent is not None:
        #         h = nbf2rnnent[h]
        #         t = nbf2rnnent[t]
        #         r = nbf2rnnrel[r]
        #     edge_weight[idx] = simkgc_dict[(h, r, t)]
    ...
    def forward(self, data, batch, edge_weight=None):
        h_index, t_index, r_index = batch.unbind(-1)
        if self.training:
            # Edge dropout in the training mode
            # here we want to remove immediate edges (head, relation, tail) from the edge_index and edge_types
            # to make NBFNet iteration learn non-trivial paths
            data, mask = self.remove_easy_edges(data, h_index, t_index, r_index)

        # edge_weight = torch.FloatTensor(edge_weight).to(r_index.device)
        # if self.training:
        ...
        # output = self.bellmanford(data, h_index[:, 0], r_index[:, 0], edge_weight)  # (num_nodes, batch_size, feature_dim）
