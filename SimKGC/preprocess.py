...
def _normalize_relations(examples: List[dict], normalize_fn, is_train: bool):
    ...
    # relation_id_to_str = json.load(open('{}/relations.json'.format(os.path.dirname(args.train_path)), 'r'))
    # for ex in examples:
    #     ex['relation'] = relation_id_to_str[ex['relation']]
    _check_sanity(relation_id_to_str)
...
def get_mappings(task):
    ...
    relations = {}
    entities = {}
    with open(path + 'entities.dict', 'r') as fi:
        for line in fi:
            line = line.strip().split('\t')
            entities[line[1]] = int(line[0])
    with open(path + 'relations.dict', 'r') as fi:
        for line in fi:
            line = line.strip().split('\t')
            if (line[1][0] != '!'):
...
def preprocess_other(path, task):
    entities, relations = get_mappings(task)
    # print(entities)
    examples = []
    with open(path, 'r') as fi:
        for line in fi:
            fs = line.strip().split('\t')
            head, relation, tail = fs[0], fs[1], fs[2]
            if (relation[0] == '!'):
                continue
            example = {'head_id': str(entities[head]),
               'head': head,
               'relation': " ".join(relation.split('_')),
               'tail_id': str(entities[tail]),
               'tail': tail} 
...
def preprocess_codex(path):
    entities = json.load(open('data/CodexM/entities.json', 'r'))
    relations = json.load(open('data/CodexM/relations.json', 'r'))
    # print(entities)
    id2ent = {_["entity_id"] : _["entity"] for _ in entities}
    examples = []
    with open(path, 'r') as fi:
        for line in fi:
            fs = line.strip().split('\t')
            head, relation, tail = fs[0], fs[1], fs[2]
            example = {'head_id': fs[0],
               'head': id2ent[fs[0]],
               'relation': relations[fs[1]],
               'tail_id': fs[2],
               'tail': id2ent[fs[2]]} 
            examples.append(example)
...
def main():
    ...
    for path in [args.train_path, args.valid_path, args.test_path]:
        ...
        elif args.task.lower() in ['umls', 'kinship', 'yago3-10']:
            all_examples += preprocess_other(path, args.task.lower())
        elif args.task.lower() in ['codexm']:
            all_examples += preprocess_codex(path)
        else:
    ...
    if args.task.lower() == 'wn18rr':
        ...
        entities = json.load(open('data/CodexM/entities.json', 'r'))
        id2text = {_["entity_id"] : _["description"] for _ in entities}
    else:
        assert False, 'Unknown task: {}'.format(args.task)
